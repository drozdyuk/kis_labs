...
> **Організаційна примітка.** Робота розрахована на 2–3 заняття. Рекомендований розподіл: перше заняття — Частина I (розгортання Docker, створення бази, встановлення Sales); друге заняття — Частина II (модуль student_module з правами доступу); третє заняття — Частина III (криптографічний модуль). Частину I доцільно виконати самостійно до першого заняття, щоб на парі зосередитися на модульній архітектурі.
...
**Від теорії до бібліотеки.** У практичній частині ми будемо використовувати бібліотеку `cryptography` для Python, яка інкапсулює всі описані кроки в одному виклику `sign()`. Під капотом бібліотека виконує саме те, що описано вище: обчислює SHA-256 хеш від payload, застосовує доповнення (padding scheme PKCS#1 v1.5) і підносить результат до степеня закритого ключа за модулем *n*. Аналогічно `verify()` виконує зворотну операцію з відкритим ключем. Розуміння математики дозволяє осмислено працювати з бібліотекою, навіть коли деталі приховані за абстракцією.
...
- `command: ["odoo", "--dev=xml"]` — режим розробника на рівні сервера; зміни у XML-файлах views застосовуються без повного перезапуску контейнера. *Зверніть увагу:* цей режим автоматично перезавантажує лише XML-шаблони. Зміни у Python-коді (моделі, методи) потребують перезапуску контейнера: `docker restart odoo19-app`. *Для продуктивного середовища параметр `--dev` слід видалити.*
...
Зверніть увагу: на відміну від `student_module`, тут немає каталогу `security/` та файлу `ir.model.access.csv`. Це тому, що модуль не створює нових моделей — він лише розширює існуючу модель `sale.order`, права доступу до якої вже визначені в модулі `sale`. Нові поля (`digital_signature`, `signature_date`, `is_verified`) автоматично успадковують права доступу батьківської моделі.
...
Модуль залежить від `sale_management`, оскільки саме цей модуль встановлює повний інтерфейс замовлень на продаж (меню, форми, звіти). Базовий модуль `sale` містить лише модель `sale.order` без UI. `"application": False` вказує, що це розширення існуючого модуля, а не самостійний додаток.
...
> **⚠ Навчальне спрощення.** У цій лабораторній використовується одна глобальна пара ключів, доступна через `ir.config_parameter`. У промисловій системі це неприпустимо з двох причин: (1) закритий ключ зберігається у відкритому вигляді в БД і доступний будь-якому адміністратору через UI (Settings → Technical → Parameters); (2) одна пара ключів на всю систему означає, що підпис не прив'язаний до конкретного користувача, тобто не забезпечує ні автентичності, ні неспростовності. У реальних системах закриті ключі зберігаються в апаратних модулях безпеки (HSM), сховищах секретів (HashiCorp Vault) або на рівні індивідуальних токенів/смарт-карт користувачів.
- **Відсутність подвійного хешування.** Метод `private_key.sign(payload, padding, hashes.SHA256())` самостійно обчислює SHA-256 хеш від payload перед підписом. Окреме попереднє хешування через `hashlib` було б помилкою: підписувався б хеш від хешу, а верифікація порівнювала б хеш від оригіналу з хешем від хешу — і завжди провалювалася б.
- **Payload із роздільниками.** Символ `|` між полями запобігає колізіям (наприклад, `"AB|C" ≠ "A|BC"`). Зверніть увагу: payload включає лише заголовкові поля замовлення (`name`, `date_order`, `amount_total`, `partner_id`). Зміна суми буде виявлена, але зміна опису рядка без впливу на суму — ні. У промисловій реалізації payload включав би серіалізацію всіх рядків замовлення (product_id, кількість, ціна).
- **Нотифікації.** Повідомлення користувачу надсилаються через `bus.bus._sendone()` — механізм серверних push-нотифікацій Odoo. Перевага перед поверненням `ir.actions.client` з `display_notification` полягає у тому, що форма коректно перезавантажується після виконання дії, а нотифікація відображається незалежно.
...
15.7. Повний код модуля `document_sign` із поясненням архітектурних рішень: збереження ключів у `ir.config_parameter` (переваги, недоліки та відмінності від промислового підходу), відсутність подвійного хешування, формування payload та його обмеження, механізм xpath для розширення views.
